const TeamCard = ({ team }) => {
  return (
    <>
      <div className="text-sm w-[300px] h-[175px] border-[1px] p-[16px] border-[#E4E7EC] box-border">
        <div className="grid grid-rows-3">
          {/* Card Header */}
          <div className="flex w-full pb-[16px]">
            <div className="w-[36px] h-[36px] mr-[9px] object-cover">
              <img alt="team" className="rounded" src={team.image} />
            </div>
            <div className="flex-grow">
              <div className="font-bold">{team.name}</div>
              <div className="text-xs opacity-[0.45]">
                {team.is_archived ? "Archived on" : "Created on"}{" "}
                {team.created_at ? team.created_at : "5 June 2022"}
              </div>
            </div>
            <div>
              {team.is_favorited ? (
          ...
          {/* Description */}
          <div className="text-xs text-[#565656] border-b-[1px] border-[#E4E7EC] ">
            <div className="h-[32px] line-clamp-2">{team.description}</div>
          </div>
          {/* Footer */}
          <div className="text-xs flex content-center items-center gap-2 opacity-[0.45]">
            ...
            <span className="flex items-center gap-1">
              ...
              {team.leads_count.toLocaleString()} Leads
            </span>
          </div>
        </div>
      </div>
